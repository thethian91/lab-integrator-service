...
class HL7Engine:
    ...
    def get_field(self, field_val: str, comp_idx: Optional[int]=None, sub_idx: Optional[int]=None) -> str:
        if comp_idx is None:
            return field_val
    ...
    def parse_path(self, path: str):
        ...
        return seg, int(field), int(rep) if rep else 1, int(comp) if comp else None, int(sub) if sub else None
    ...
    def get_value_from_hl7(self, hl7_msg: str, path: str) -> str:
        ...
        segments = [s for s in self.split_segments(hl7_msg) if s.startswith(seg_name + self.sep["field"])]
        if not segments:
            return ""
        seg = segments[0]
    ...
    def _parse_guess_datetime(self, text: str) -> Optional[dt.datetime]:
        for fmt in ("%Y-%m-%d", "%Y-%m-%d %H:%M:%S", "%Y%m%d", "%Y%m%d%H%M%S", "%d/%m/%Y", "%d/%m/%Y %H:%M:%S"):
            try:
    ...
    def _render_line(self, raw_line: str, data: Dict[str, Any], hl7_in: Optional[str], this: Optional[Dict[str, Any]]=None) -> str:
        line = raw_line
    ...
    def extract(self, profile: str, hl7_msg: str) -> Dict[str, Any]:
        ...
        for r in rules:
            ...
            # datefmt_in/out en pareja (si se usan aquí)
            t_in = next((t.split(":",1)[1] for t in transforms if t.startswith("datefmt_in:")), None)
            t_out = next((t.split(":",1)[1] for t in transforms if t.startswith("datefmt_out:")), None)
            if t_in and t_out:
                val = self.apply_date_in_out(val, t_in, t_out)
    ...
    def _get_field_from_parts(self, parts: List[str], field_idx: int,
                              comp_idx: Optional[int], sub_idx: Optional[int]) -> str:
    ...
    def extract_grouped(self, profile: str, hl7_msg: str, base_out: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        ...
        obr_fields = cfg.get("fields", {})     # dict con "obr.xxx": "OBR-4-1" o {"path": "...", "transforms":[...]}
        child = cfg["children"]
        ...
        for seg_name, parts in self.iter_segments(hl7_msg):
            if seg_name == base_seg:
                ...
                for name, spec in obr_fields.items():
                    ...
                    val = self.apply_transforms(val, [t for t in transforms if not t.startswith("datefmt_")])
                    self._assign_to_dict_path(out, assign_root.replace("[*]", f"[{current_group_index}]") + "." + name, val)
                ...
                base_path = assign_root.replace("[*]", f"[{current_group_index}]") + "." + child_assign.replace("[*]", f"[{child_index}]")
